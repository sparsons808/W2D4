...
def reverse_string(str)
    ...
    reversed = reverse_string(str[1..-1]) + str[0]
    reversed
end
...
def flatten(data)
    if !Array.try_convert(data) || data.empty?
        return [data]
    end
    flattened = []
    data.each do |ele|
        if ele = Array.try_convert(ele)
            flatten(ele)
        else
            flattened << ele
        end
...
def flatten(data, level = 1)
    ...
    data.each do |el| 
        ...
        flattened_el = flatten(el, level + 1)
        debugger
